...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [
      ...
      supabaseAdmin.from('integrations').select('id, broker_id, integration_type, is_active, last_sync_at, page_id, sync_status, webhook_url, created_at, updated_at').eq('broker_id', brokerId),
      supabaseAdmin.from('portal_connections').select('id, broker_id, portal_name, api_url, is_active, last_sync_at, sync_status, created_at, updated_at').eq('broker_id', brokerId),
