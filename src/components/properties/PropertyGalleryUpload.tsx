...
export const PropertyGalleryUpload = ({
  ...
  const uploadImage = async (file: File): Promise<string | null> => {
    ...
    try {
      ...
      const fileName = `${userId}/${propertyId || 'new'}/gallery/${Date.now()}_${Math.random().toString(36).substr(2, 9)}.${fileExt}`;
          ...
          {isUploading ? (
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-sm text-muted-foreground">
                {isDragging ? 'Solte para enviar' : 'Arraste fotos das áreas comuns ou'}
              </p>
              <Button
                type="button"
                variant="outline"
