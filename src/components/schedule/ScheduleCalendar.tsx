...
function ScheduleCalendar({ 
  ...
  // Custom DayContent component to show event counts
  const DayWithEvents = (props: DayContentProps) => {
    const { date, displayMonth } = props;
    const count = getEventCount ? getEventCount(date) : 0;
  ...
  const compactClassNames: DayPickerProps['classNames'] = compact ? {
    ...
    day: cn(
      "h-10 w-full p-2 font-normal aria-selected:opacity-100",
  ...
  return (
    <DayPicker
      ...
      components={{
        ...
        DayContent: getEventCount ? DayWithEvents : undefined,
      }}
    />
  );
}
